...
# NEW: Expert Consultation Tools
def consult_expert1(query: str, max_tokens: int = 500) -> str:
    """Consult Expert 1 (Grok-4) for a second opinion."""
    try:
        client = OpenAI(api_key=API_KEY, base_url="https://api.x.ai/v1")
        sys_prompt = "You are Expert 1, a specialized AI providing concise, analytical advice. Focus on the query without tool calls."
        ...
        return f"Expert 1 Opinion: {response.choices[0].message.content.strip()}"
    except Exception as e:
        return f"Expert 1 error: {str(e)}"

def consult_expert2(query: str, max_tokens: int = 500) -> str:
    """Consult Expert 2 (Grok-4) for a second opinion."""
    try:
        client = OpenAI(api_key=API_KEY, base_url="https://api.x.ai/v1")
        sys_prompt = "You are Expert 2, a specialized AI providing concise, creative advice. Focus on the query without tool calls."
        response = client.chat.completions.create(
            model="grok-4-0709",
            messages=[{"role": "system", "content": sys_prompt}, {"role": "user", "content": query}],
            max_tokens=max_tokens,
        ...
        return f"Expert 2 Opinion: {response.choices[0].message.content.strip()}"
    except Exception as e:
        return f"Expert 2 error: {str(e)}"
...
TOOLS = [
    ...
    # NEW: Expert 1 Tool
    {
        "type": "function",
        "function": {
            "name": "consult_expert1",
            "description": "Consult Expert 1 (a Grok-4 instance) for a second opinion or expert advice. Use for complex decisions, debugging, or validation.",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "The question or task to consult on."},
                    "max_tokens": {"type": "integer", "description": "Max tokens for response (default 500)."}
                },
                "required": ["query"]
            }
        }
    ...
    # NEW: Expert 2 Tool
    {
        "type": "function",
        "function": {
            "name": "consult_expert2",
            "description": "Consult Expert 2 (a Grok-4 instance) for a second opinion or expert advice. Use for complex decisions, debugging, or validation.",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "The question or task to consult on."},
        ...
        while iteration < max_iterations:
            ...
            for tool_call in tool_calls:
                ...
                try:
                    ...
                    # NEW: Expert Tools
                    elif func_name == "consult_expert1":
                        query = args['query']
                        max_tokens = args.get('max_tokens', 500)
                        result = consult_expert1(query, max_tokens)
                    elif func_name == "consult_expert2":
                        query = args['query']
                        max_tokens = args.get('max_tokens', 500)
                        result = consult_expert2(query, max_tokens)
                    else:
                        result = "Unknown tool."
                except Exception as e:
